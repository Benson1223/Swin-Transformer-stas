...
def main(args=None):
	...
	img_list = []
	temp = ""
	with open(parser.csv_val, newline='') as csvfile:
		rows = csv.reader(csvfile,delimiter=',')
		for row in rows:
			img_file, x1, y1, x2, y2, class_name = row[:6]
			img_file_list = img_file.split("/")
	...
	for idx, data in enumerate(dataloader_val):
		...
		with torch.no_grad():
			...
			for j in range(idxs[0].shape[0]):
				...
				img_score = ('%.5f'%scores[idxs[0][j]])
				cv2.rectangle(img, (x1, y1), (x2, y2), color=(0, 0, 255), thickness=2)
				
				if(i==0):
					bbox_str = bbox_str+"["+str(x1)+","+str(y1)+","+str(x2)+","+str(y2)+","+img_score+"]"
				else:
					bbox_str = bbox_str+",["+str(x1)+","+str(y1)+","+str(x2)+","+str(y2)+","+img_score+"]"
				# print(bbox_list)
				print(bbox_str)
			...
			# cv2.imshow('img', img)
			cv2.imwrite('./output/'+img_list[k]+'.jpg', img)
			k = k+1
			# print(dataset_val.image_names[])
			# cv2.waitKey(0)
